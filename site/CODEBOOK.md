...
There are 78 columns: 13 participant/session columns + 2 parts × 4 conditions × 8 episode columns + `detail_json`.
The same list, one row per column, is in `codebook.csv`.

**Conventions**
- All `_ms` variables are **milliseconds**, rounded to whole numbers. Divide by 1000 for seconds.
- **Active time**: timers run only while the browser tab is visible. They pause if the participant switches tabs, minimizes the window, or locks the phone.
  `total_ms` is the only exception: it is wall-clock time.
...
| `total_ms` | integer, ms | ≥ 0 | `finished_at` − `started_at`. Wall-clock time: it **includes** time spent in other tabs. |
...
| `set1_ms` | integer, ms | ≥ 0 | Active time in Part 1: all episode cards plus all Read More pages. Instructions are not included. |
| `set2_ms` | integer, ms | ≥ 0 | Same as `set1_ms`, for Part 2. |
| `set1_instructions_ms` | integer, ms | ≥ 0 | Active time on the Part 1 instructions page.\* |
| `set2_instructions_ms` | integer, ms | ≥ 0 | Active time on the Part 2 instructions page.\* |
...
| `_title_view_ms` | integer, ms | ≥ 0 | **Title reading time.** Total active time this episode's card (title + one-sentence summary + Read More link) was on screen, summed over every visit to the card. Time on the Read More page is **not** included. |
...
| `_readmore_ms` | integer, ms | ≥ 0 | **Read More reading time.** Total active time on this episode's full-description page, summed over all visits. 0 if never opened. |
...
```
{
  ...
  "metrics": { "set1": { "<episode id>": {
                 "title_view_ms", "title_views", "readmore_clicks", "readmore_ms",
                 "readmore_visits": [ { "opened_at", "ms", "max_scroll_pct" }, ... ] } }, "set2": {...} },
  "events":  [ { "t_ms", "type", "set", ...extra }, ... ]
}
```

- `metrics` values here are **unrounded** milliseconds.
...
- `events` is a timeline of what the participant did. `t_ms` = milliseconds since `started_at` (wall-clock). `set` = the stage when the event happened: `id`, `set1`, `set2`, or `done`.
...
| `readmore_close` | `episode`, `visit_ms` | Participant leaves the Read More page |
...
- **Reshaping to long format.** For models with repeated measures, reshape to 8 rows per participant (2 parts × 4 conditions). Use `pid`, `set`, and `condition` as keys, with `episode`, `position`, `title_view_ms`, `readmore_clicked`, and `readmore_ms` as columns.
- **Selective exposure DVs.** The main ones are `_title_view_ms` (attention to the title), `_readmore_clicked` (choosing to read more), and `_readmore_ms` (time reading the full description).
  `_readmore_ms` is 0 whenever `_readmore_clicked` = 0, so consider a two-part (hurdle) model or analyze it among clickers only.
- **Controls.** Consider `position` (order effects), `episode` (stimulus item effects), and `touch_device`.
- **Data-quality checks.**
  - Very short `title_view_ms` on all cards may mean the participant clicked through quickly.
  - Many `tab_hidden` events, or a large gap between `total_ms` and the sum of active times, suggests the participant was multitasking.
